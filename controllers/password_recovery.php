...
{	
	...
	{
		$this->template->setTitle("Password recovery by email");

		/* daca input-ul nu e gol */
		if($this->input->post('recover_email'))
		{
			$email = $this->input->post('recover_email');
			/* ia username-ul din DB */
			$username = $this->password_recovery_model->getUsername($email);
			
			/* daca exista username-ul */
			if($username)
			{
				/* genereaza link unic */
				$link = base_url().'password_recovery/requestPassword/email/'.$this->generateKey($username, $email);
				/* trimite mail cu tot cu username */
				sendMail($email, $this->config->item('password_recovery_sender_email'), $this->config->item('server_name').': '.lang("reset_password", "recovery"), lang("email", "recovery").' <a href="'.$link.'">'.$link.'</a><br> Your username is: <strong>' . $username . '<strong>');
